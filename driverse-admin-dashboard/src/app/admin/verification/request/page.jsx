...
const AdminVerificationRequest = () => {
  ...
  const handleVerification = async (userId, documentType) => {
    setLoading(true)
    try {
      const response = await fetch(`/api/verifyDoc?userId=${userId}&documentType=${documentType}`, {
        method: 'POST',
        headers: {
  ...
  const getDocuments = (doc) => {
    ...
    return [
      { id: doc.userId, verify: doc?.verify_articleOfIncorporation.verified, doc_name: 'articleOfIncorporation', label: "Article of Incorporation", url: doc?.articleOfIncorporation },
      { id: doc.userId, verify: doc?.verify_wsibCertificate.verified, doc_name: 'wsibCertificate', label: "WSIB Certificate", url: doc?.wsibCertificate },
      { id: doc.userId, verify: doc?.verify_insuranceCertificate.verified, doc_name: 'insuranceCertificate', label: "Insurance Certificate", url: doc?.insuranceCertificate },
      { id: doc.userId, verify: doc?.verify_operatingAuthorities.verified, doc_name: 'operatingAuthorities', label: "Operating Authorities", url: doc?.operatingAuthorities },
    ].filter(doc => doc.url);
  };
  ...
  // Verify KYC document
  const verifyKYC = async (kycDoc) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`/api/kyc/${kycDoc}`, null, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
                    ...
                    <tr key={kycDoc.userId || Math.random()} className="border-b hover:bg-gray-50">
                      ...
                      <td className="py-2 px-2 sm:px-4">
                        ...
                        >
                          ...
                          {/* {kycDoc?.articleOfIncorporation?.verified ? 'Verified' : 'Verify'} */}
                        </button>
                      </td>
                      <td className="py-2 px-2 sm:px-4">
                        <button
